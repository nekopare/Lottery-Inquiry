...
function buildSystemPrompt({ lotteryName, lotteryType, recentResults, promptStyle }) {
  ...
  const framework = isSSQ
    ? [
        "一、冷热号分析：统计红球33个号码和蓝球16个号码在近100期的出现频次；定义出现≥16次为热号，8-15次为温号，≤7次为冷号；以表格列出当前热号、温号、冷号清单，并标注上期回补情况。",
    ...
    : [
        "一、冷热号分析：统计前区35个号码和后区12个号码在近100期的出现频次；定义出现≥16次为热号，8-15次为温号，≤7次为冷号；以表格列出当前热号、温号、冷号清单，并标注上期回补情况。",
